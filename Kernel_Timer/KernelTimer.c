...
#define TIMEOUT 5000 //milisecond // after 5 secs since start kernel module
...
void timer_callback(struct timer_list * timer){
    ...
    // Re-init timer which will change periodic timer
...
static int __init chr_driver_init(void){
    ...
    // setup timer to call callback function
    timer_setup(&chr_timer,timer_callback,0);

    // Setup timer interval base on TIMEOUT
