...
char chr_array[] = "Hi. How is your day going ?";
...
static ssize_t read_proc(struct file *filp, char __user *buffer, size_t len, loff_t *offset);
static ssize_t write_proc(struct file *filp, const char *buffer, size_t len, loff_t *offset);
...
static ssize_t read_proc(struct file *filp, char __user *buffer, size_t len, loff_t *offset){
    ...
    copy_to_user(buffer,chr_array,sizeof(chr_array));
    return len;
}

static ssize_t write_proc(struct file *filp, const char *buffer, size_t len, loff_t *offset){
    printk(KERN_INFO "Profs file is writing.\n");
    copy_from_user(chr_array,buffer,len);
    return len;
}
